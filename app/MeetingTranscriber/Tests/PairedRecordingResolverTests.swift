...
final class PairedRecordingResolverTests: XCTestCase {
    ...
    func testFullTriplet() {
        let result = PairedRecordingResolver.resolve(urls: [
            url("standup_audio_app.wav"),
            url("standup_audio_mic.wav"),
            url("standup_audio_mix.wav"),
        ])
        ...
        XCTAssertEqual(group.mix?.lastPathComponent, "standup_audio_mix.wav")
    }

    func testMixOnlyIsPaired() {
        let result = PairedRecordingResolver.resolve(urls: [
            url("meeting_audio_mix.wav"),
        ])
        ...
        XCTAssertEqual(group.mix?.lastPathComponent, "meeting_audio_mix.wav")
    ...
    func testAppPlusMicWithoutMixIsPaired() {
        ...
        let result = PairedRecordingResolver.resolve(urls: [
            url("20260311_143000_audio_app.wav"),
            url("20260311_143000_audio_mic.wav"),
        ])
        ...
        XCTAssertEqual(group.app?.lastPathComponent, "20260311_143000_audio_app.wav")
        XCTAssertEqual(group.mic?.lastPathComponent, "20260311_143000_audio_mic.wav")
    }
    ...
    func testLoneAppFallsBackToSingleton() {
        let result = PairedRecordingResolver.resolve(urls: [
            url("alone_audio_app.wav"),
        ])

        XCTAssertEqual(result.paired.count, 0)
        XCTAssertEqual(result.singletons.count, 1)
        XCTAssertEqual(result.singletons[0].lastPathComponent, "alone_audio_app.wav")
    }

    func testLoneMicFallsBackToSingleton() {
        let result = PairedRecordingResolver.resolve(urls: [
            url("alone_audio_mic.wav"),
        ])

        XCTAssertEqual(result.paired.count, 0)
        XCTAssertEqual(result.singletons.count, 1)
        XCTAssertEqual(result.singletons[0].lastPathComponent, "alone_audio_mic.wav")
    }
    ...
    func testOnePairPlusOneSingleton() {
        let result = PairedRecordingResolver.resolve(urls: [
            url("meeting_audio_app.wav"),
            url("meeting_audio_mic.wav"),
            url("meeting_audio_mix.wav"),
            url("standalone.mp3"),
        ])
    ...
    func testTwoSeparatePairs() {
        let result = PairedRecordingResolver.resolve(urls: [
            url("aaa_audio_app.wav"), url("aaa_audio_mic.wav"), url("aaa_audio_mix.wav"),
            url("bbb_audio_app.wav"), url("bbb_audio_mic.wav"), url("bbb_audio_mix.wav"),
        ])
    ...
    func testSameStemDifferentDirectoriesAreSeparateGroups() {
        ...
        let result = PairedRecordingResolver.resolve(urls: [
            dirA.appendingPathComponent("meeting_audio_app.wav"),
            dirA.appendingPathComponent("meeting_audio_mic.wav"),
            dirA.appendingPathComponent("meeting_audio_mix.wav"),
            dirB.appendingPathComponent("meeting_audio_app.wav"),
            dirB.appendingPathComponent("meeting_audio_mic.wav"),
            dirB.appendingPathComponent("meeting_audio_mix.wav"),
        ])
    ...
    func testResolverNeverAliasesMixWithAppOrMic() {
        // When a `_audio_mix.wav` is part of the selection, it must be distinct from
        // app/mic by suffix construction. Groups without a mix carry `nil`,
        // and the synthesizer creates a real mix file at enqueue time.
        let cases: [[URL]] = [
            [url("a_audio_app.wav"), url("a_audio_mic.wav"), url("a_audio_mix.wav")],
            [url("b_audio_mix.wav")],
            [url("c_audio_app.wav"), url("c_audio_mix.wav")],
            [url("d_audio_mic.wav"), url("d_audio_mix.wav")],
        ]
        for urls in cases {
    ...
    func testGroupOrderFollowsFirstURLIndex() {
        ...
        let result = PairedRecordingResolver.resolve(urls: [
            url("early_audio_mix.wav"), // 0 — pair "early" earliest member
            url("solo.mp3"), // 1 — singleton
            url("late_audio_mix.wav"), // 2 — pair "late" earliest member
            url("early_audio_app.wav"), // 3 — pair "early" later member (irrelevant for order)
            url("late_audio_mic.wav"), // 4
        ])
